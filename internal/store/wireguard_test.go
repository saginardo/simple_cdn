...
func TestWireGuardTunnelEnrollmentSiteReferencesAndPerformance(t *testing.T) {
	...
	}, []string{otherNode.ID}); err == nil || !strings.Contains(err.Error(), "overlaps") {
	...
	if _, err := database.UpdateWireGuardTunnel(configured, []string{otherNode.ID}); !errors.Is(err, ErrWireGuardTunnelInUse) {

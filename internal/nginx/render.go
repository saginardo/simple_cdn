...
const CompressionRuntimeMarker = "# Compression runtime v1"

const configTemplate = `# Generated by cdn-edge-agent. Do not edit.
{{if .Sites}}map_hash_bucket_size 128;
...
{{range .Sites}}{{$site := .}}{{if .StaticAssets}}map $uri ${{.StaticBytesVariable}} {
    default 0;
{{range .StaticAssets}}    "{{.URLPath}}" {{.SizeBytes}};
{{end}}}
{{end}}{{end}}
...
{{end}}
    server_name {{.DomainList}};
	set $cdn_site_id "{{.ID}}";
{{if .StaticAssets}}	set $cdn_static_uncompressed_bytes ${{.StaticBytesVariable}};
{{end}}
    ...
    gzip_min_length 256;
    gzip_comp_level 6;
    gzip {{if .DynamicCompressionEnabled}}on{{else}}off{{end}};
    gzip_types {{.CompressionTypes}};
{{if $.CompressionCapable}}    brotli_static on;
    brotli_min_length 256;
    brotli_comp_level 5;
    brotli {{if .DynamicCompressionEnabled}}on{{else}}off{{end}};
    brotli_types {{.CompressionTypes}};
    zstd_static on;
    zstd_min_length 256;
    zstd_comp_level 3;
    zstd {{if .DynamicCompressionEnabled}}on{{else}}off{{end}};
    zstd_types {{.CompressionTypes}};
{{end}}
{{if .CacheEnabled}}    set $cdn_cache_scope_generation "0";
{{range .CacheInvalidations}}{{if eq .Scope "url"}}    if ($request_uri = "{{.Value}}") { set $cdn_cache_scope_generation "{{.Generation}}"; }
{{else}}    if ($uri ~ "^{{.Regex}}") { set $cdn_cache_scope_generation "{{.Generation}}"; }
{{end}}{{end}}{{end}}
    {{if $.LegacySecurityEnabled}}access_log /opt/cdn-edge/logs/security.json cdn_security_json if=$cdn_security_policy_id;
    if ($cdn_security_policy_id) { return 444; }
    {{end}}
	...
	proxy_cache_key "{{.ID}}:{{.CacheGeneration}}:$cdn_cache_scope_generation:$scheme$host$request_uri";
...
type renderedSite struct {
	...
	CacheInvalidations           []renderedCacheInvalidation
	GRPC                         bool
	Passthrough                  bool
	HTTP3Enabled                 bool
	IPv6Enabled                  bool
	...
	DynamicCompressionEnabled    bool
	CompressionTypes             string
	StaticAssets                 []renderedStaticAsset
	StaticBytesVariable          string
}
...
type renderedCacheInvalidation struct {
	Scope      string
	Value      string
	Regex      string
	Generation int64
...
func RenderWithRuntimeOptions(sites []domain.Site, securityPolicies []domain.SecurityPolicy, rateLimitPolicies []domain.RateLimitPolicy, options RenderRuntimeOptions) (string, error) {
	...
	for _, reference := range options.StaticAssets {
		...
		staticAssetsBySite[reference.SiteID] = append(staticAssetsBySite[reference.SiteID], renderedStaticAsset{
			...
			ContentType: reference.ContentType, CacheControl: reference.CacheControl, SizeBytes: reference.SizeBytes,
		})
	}
	...
	for _, site := range sites {
		...
		compressionExclusions, err := domain.NormalizeCompressionExcludedMIMETypes(site.CompressionExcludedMIMETypes)
		if err != nil {
			return "", fmt.Errorf("site %s: %w", site.Name, err)
		}
		compressionTypes := domain.DynamicCompressionMIMETypes(compressionExclusions)
		if len(compressionTypes) == 0 {
			compressionTypes = []string{"application/x-cdn-no-compression"}
		}
		cacheInvalidations, err := domain.NormalizeCacheInvalidationRules(site.CacheInvalidations)
		if err != nil {
			return "", fmt.Errorf("site %s: %w", site.Name, err)
		}
		renderedInvalidations := make([]renderedCacheInvalidation, 0, len(cacheInvalidations))
		for _, rule := range cacheInvalidations {
			renderedInvalidations = append(renderedInvalidations, renderedCacheInvalidation{
				Scope: string(rule.Scope), Value: rule.Value, Regex: regexp.QuoteMeta(rule.Value), Generation: rule.Generation,
			})
		}
	...
	if err := template.Execute(&out, renderInput{
		...
		CompressionCapable:            options.CompressionCapable,
	}); err != nil {
		return "", err
	}

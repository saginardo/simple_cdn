...
- Cloudflare DNS-only A/AAAA reconciliation after node reachability and independent per-site IPv4/IPv6 health hysteresis. IPv6 is opt-in per site and only nodes with a configured, healthy public IPv6 receive AAAA records. A site may predeploy to standby nodes that stay outside DNS during normal operation; healthy standbys enter DNS only after every primary is unavailable, and any recovered primary automatically replaces them. Three failed probes remove an address and five successful probes restore it. If both IPv4 pools are unhealthy, DNS is deliberately left unchanged.
...
- Single administrator, required public IPv4 with optional public IPv6 per node, Cloudflare DNS-only, a single Cloudflare account, no tenant/RBAC model, no GeoDNS, no managed bot-reputation/CAPTCHA or volumetric DDoS service, and no control-plane high availability.
...
1. Add a node in the **Nodes** view with its fixed public IPv4 and, when available, its optional public IPv6.
...
3. Start uninstall preparation. The controller removes only Cloudflare A and AAAA records whose managed comment exactly identifies that node, then enforces a 75-second DNS safety wait.
...
5. Wait for an edge to be active and pass five node and per-site probes. The controller creates DNS-only A records and, for an opted-in site, independently creates AAAA records after five successful IPv6 probes. Both families use the site's published TTL override or the global default of 60 seconds.
...
```text
Client -> Cloudflare DNS-only A/AAAA -> Edge Nginx -> primary origin -> optional backup origin
                                      |

...
- 状态：后台定期汇总节点、站点、发布、证书、备份与升级状态，提供过期提示、判断证据、对象处理入口及持久恢复记录。详见 [状态说明](docs/HEALTH_CENTER.md)。
...
具备 `machine_status_adaptive_v1` 能力的代理在主控通过 mTLS SSE 策略流成功协商自适应采样前，仍保持旧版 5 秒完整快照上报。协商成功且没有可见的节点详情页时，完整主机快照调整为每 60 秒一次，回源健康与连接运行态则独立保持 5 秒上报。可见的节点详情页建立受认证的 SSE 连接后，策略会把完整主机快照提高到每 5 秒一次，并启用轻量网络采样：代理先建立默认路由网卡计数器基线，再每秒上报 RX/TX 区间速率。最后一个可见订阅者离开且 15 秒重连宽限期结束后，网络高速采样停止，完整主机快照恢复到 60 秒，5 秒回源运行态继续运行。旧主控缺少策略端点或主控回滚时，代理会静默恢复 5 秒完整快照，不会把不支持的新接口写入节点错误。已成功协商策略后，如果策略流发生短暂传输故障，代理会在 60 秒恢复宽限期内保留最后一份有效频率；EOF 或读超时最多每 15 秒重连一次，且宽限期内不把该故障写入心跳。宽限期耗尽后才回退到 5 秒完整快照并上报错误，收到下一份有效策略时自动清除。旧的 `machine_status_stream_v1` 代理继续保持原有固定频率。常规 30 秒心跳也会携带最新完整快照。主控只在内存中保存最新主机、回源与网络快照，页面保留 30 秒详情刷新作为断线兜底。机器状态不持久化历史数据，尚未收到完整快照时页面不显示对应区域。节点详情显示 Linux 发行版和版本、运行时长、1/5/15 分钟负载、逻辑 CPU 数与区间利用率、内存和根文件系统用量、默认路由接口的 RX/TX 速率，以及有样本时的回源池状态。

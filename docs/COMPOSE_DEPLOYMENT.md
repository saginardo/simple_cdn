...
When the job reaches **Ready**, live data is still unchanged. Committing requires the exact text `RESTORE`. The controller waits for active backup and certificate operations, writes a maintenance marker, exits cleanly, and applies the verified hashes at the next startup before opening SQLite. It promotes the temporary ClickHouse database, swaps SQLite (including stale WAL/SHM handling), internal CA, site Certbot state, control TLS and `nginx-artifacts`, then retains the previous files and ClickHouse database under job-specific rollback names. The current deployment environment files remain authoritative and are not restored by the online path. The Compose restart policy brings the controller back; the UI reconnects and the message center records the terminal state. Managed static-resource object bytes are not included in the Restic archive and are not recreated by this workflow.

...
Docker Compose is the supported control-plane deployment. Edge nodes run the project-managed Nginx bundle and a systemd Agent directly on Debian, with CDN-owned files consolidated below `/opt/cdn-edge`; see [EDGE_DEPLOYMENT.md](EDGE_DEPLOYMENT.md).
...
/opt/cdn-platform/
  ...
  data/
    control/                   SQLite, internal CA, site Certbot state,
                              static-assets/objects, nginx-artifacts
  ...
  logs/
    ...
    clickhouse/
  backup/
    staging/control/           SQLite, control archives and validated
                              managed static-resource objects
...
An optional host-network reverse proxy stays outside this directory. It proxies the management URL from public port 443 to the controller's direct TLS listener (the tracked example uses `https://127.0.0.1:8443`). The control container terminates TLS itself so edge mTLS remains end-to-end; do not send edge client certificates through the management proxy.
...
Create a stable release from the tested commit without editing any version file (replace the placeholders with the next semantic version):

```bash
release=vMAJOR.MINOR.PATCH
git tag -a "$release" -m "simple_cdn $release"
...
Configure GHCR package visibility and pull credentials to match the deployment policy. Actions publishes with `GITHUB_TOKEN`; a private package requires the deployment host to authenticate pulls with an account or token that has package-read permission. Do not put that credential in this repository.
...
The same control container runs the managed Nginx release checker; no additional service or image is required. By default it polls the public `saginardo/simple_cdn` GitHub Releases API every 24 hours and stores verified bundles in `/var/lib/cdn-platform/nginx-artifacts`, which is already inside the writable `./data/control` volume. Configure a fork with `NGINX_UPDATE_GITHUB_REPOSITORY=owner/repository`; use `NGINX_UPDATE_GITHUB_TOKEN` only when releases are private or unauthenticated API quota is insufficient. `NGINX_UPDATE_ENABLED=false` disables both scheduled and manual checks without removing already approved artifacts. See [NGINX_UPDATES.md](NGINX_UPDATES.md) for the trust chain and approval procedure.

GitHub-hosted workflow artifacts are not used as the production origin. The daily stable workflow publishes fixed-name assets to a GitHub Release, while the controller downloads and redistributes the validated bundle from its own immutable `/downloads/nginx/<sha256>/...` routes. Administrators approve the candidate and start node upgrades from the **Nodes** view. This separates Nginx rollout from control-image rollout while retaining the image-bundled version as a bootstrap fallback.

ClickHouse currently uses the `clickhouse/clickhouse-server:26.6` release-line tag. A tag update does not modify a running container: apply it only through a controlled pull and recreation after a verified native backup. Any future release-line or LTS change must be validated against the schema, backup, restore and resource limits before changing the Compose image.
...
The controller independently checks freshness every minute. A previously successful run becomes stale after `BACKUP_MAX_AGE` (26 hours plus the configured random delay); running/retrying jobs become stalled after `BACKUP_STALLED_AFTER` (6 hours). Retry/failure records preserve the last success time. Stale, stalled and missing status alerts are deduplicated per run.

When backup settings and online restore are available, the controller verifies the latest `cdn-control-compose` snapshot at startup if no recent verification exists, then every `BACKUP_VERIFY_INTERVAL` (168 hours). Failed checks retry no more than once per hour. This downloads the snapshot, validates SQLite/encryption/certificates/static objects/Nginx artifacts, restores ClickHouse into a temporary database, validates it, and removes temporary data. It never commits a live cutover. Active backup runs and active/prepared manual restores defer automatic checks. **Settings > Backup and restore** shows the last success, proven snapshot time, failures, and a manual verification/cancellation action. A failed attempt preserves the previous proven recovery point in `online-restore/backup-verification.json`.

Verification uses the existing `ONLINE_RESTORE_PREPARE_TIMEOUT` and needs enough disk for the downloaded snapshot and temporary ClickHouse database. It demonstrates snapshot recoverability, not a full replacement-host boot, credential escrow, DNS/edge recovery or a measured production RTO; periodically retain that broader operational drill.


Retention is 7 daily, 4 weekly, and 6 monthly snapshots. The backup includes the `simple_cdn` ClickHouse database, not recreatable `system` diagnostic tables, plus `compose.yaml` and `.env` so the exact GHCR image reference is recorded. It also archives the internal CA, site Certbot state, control TLS, downloaded Nginx artifacts, and managed static-resource objects. The SQLite online copy is converted to a self-contained non-WAL journal mode before staging. Static objects are staged from that copy's exact metadata set, with their byte length and SHA-256 checked before Restic runs; an object deletion racing the copy therefore fails the attempt instead of producing an incomplete snapshot. Size the local `backup/staging` filesystem for the ClickHouse native backup plus one complete copy of the controller's managed static objects. Restore accepts legacy snapshots containing `cdn_platform` and promotes them under the current name. A snapshot created before static-object coverage remains usable only when its SQLite database has no managed static assets; the restore script handles that case without requiring the legacy image to provide the newer object-verification command. Otherwise validation rejects the missing objects. Backup and certificate containers take a shared operation lock, while a restore cutover takes the exclusive lock and publishes a maintenance marker so no new writer operation starts during the swap. After an image upgrade, ordinary `docker compose up -d --no-build` includes the required scheduler; the deployment script also recreates and verifies it automatically.
...
When the job reaches **Ready**, live data is still unchanged. Committing requires the exact text `RESTORE`. The controller waits for active backup and certificate operations, writes a maintenance marker, exits cleanly, and applies the verified hashes at the next startup before opening SQLite. It promotes the temporary ClickHouse database, swaps SQLite (including stale WAL/SHM handling), internal CA, site Certbot state, control TLS, managed static-resource objects and `nginx-artifacts`, then retains the previous files and ClickHouse database under job-specific rollback names. The static-object set must exactly match the restored SQLite metadata and is hashed again immediately before and after cutover. The current deployment environment files remain authoritative and are not restored by the online path. The Compose restart policy brings the controller back; the UI reconnects and the message center records the terminal state.

Only one restore job may be active. A downloading or validating job can be cancelled without touching live data; a ready job can also be cancelled and its temporary database is dropped. A committed job cannot be cancelled. Restore job state is versioned: after an image change, an active job created by an incompatible controller is failed and its temporary data is removed before any cutover, so the restore must be started again. If rollback is incomplete, the maintenance marker is retained and startup fails closed so an operator can inspect both versions rather than resume writers against a mixed state. Keep the offline script and credential record: online restore is a convenience for a healthy controller, not a replacement for bare-host disaster recovery.

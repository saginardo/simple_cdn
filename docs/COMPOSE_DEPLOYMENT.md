...
ClickHouse tracks the latest patch in the `26.6` release line through the floating `clickhouse/clickhouse-server:26.6` tag. A tag update does not modify a running container: apply it only through a controlled pull and recreation after a verified native backup. When a release newer than 26.6 is officially designated as the next LTS, validate the upgrade and restore paths before moving the floating release-line tag to that LTS; the expected next candidate is 26.8, but the official designation remains authoritative.

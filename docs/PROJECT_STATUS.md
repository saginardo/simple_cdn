...
- `wireguard_v1` 节点在本地生成并以 `0600` 持久化 Curve25519 私钥，按 ETag 拉取隧道期望状态，经 `wg-quick` 原子应用或回滚并上报握手、流量、修订与错误；端点解析到本机地址时会在改写配置前失败关闭。Agent 以 `tc` 管理边缘出口上限，源站安装器以 `wg-quick` 生命周期钩子持久化隧道总出口上限。`wireguard_performance_v1` 节点只在 3 分钟内存在握手时串行执行公网 TCP、隧道 TCP 和隧道 UDP `iperf3` 测试。详见 [WIREGUARD_ORIGIN.md](WIREGUARD_ORIGIN.md)。

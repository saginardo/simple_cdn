...
HTTP/3 is disabled by default for both new and existing sites. When a site enables it in the panel and is published to an `http3_v1` node, that node keeps `listen 443 ssl` plus `http2 on` on TCP and adds `listen 443 quic` on UDP for the opted-in site. Its HTTPS responses advertise `h3` through `Alt-Svc` for 24 hours. `quic_retry on` limits address-spoofing amplification, while TLS 0-RTT remains disabled to avoid replay semantics. Sites that leave the option off and nodes without the capability receive none of these directives, so mixed Debian 12/13 fleets can roll forward safely. Nginx upstream still labels `ngx_http_v3_module` experimental, so keep the TCP fallback and roll out one edge at a time.

The Agent treats UDP 443 as desired state rather than an incidental Nginx socket whenever at least one published site enables HTTP/3. Before applying such a configuration it rejects foreign UDP owners, after reload it waits for Nginx to own the socket, and on failure it restores the previous fragments, certificates, and applied version. Published site-option and capability changes are reconciled from subsequent heartbeats, including the first heartbeat after an online upgrade; publishing the last HTTP/3 site's disabled state removes the QUIC listener and UDP port declaration. The project-owned nftables ban table drops banned IPv4 sources on UDP 443 as well as TCP 80/443.
...
New agents advertise `origin_connection_v1`. After the controller rebuilds that node's desired state, compatible sites share managed upstream pools below `/opt/cdn-edge/config/nginx/origin-pools`; layered service/cold probe and circuit state is retained in `/opt/cdn-edge/data/origin-connections.json`. Pool include changes and desired-state reloads are serialized and use the same `nginx -t`, worker-generation confirmation, and rollback guarantees. The latest two-layer probe results travel inside the five-second machine snapshot and are not retained by the controller. Do not edit or delete individual pool files while the agent is running. Pool identity, adaptive sizing, thresholds, and diagnostics are documented in [ORIGIN_CONNECTIONS.md](ORIGIN_CONNECTIONS.md).

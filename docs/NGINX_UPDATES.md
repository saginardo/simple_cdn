...
Compose Restic 流程会归档 `nginx-artifacts`，离线恢复会随控制数据目录恢复；在线恢复也会在提交阶段原子切换该目录并保留旧目录用于回滚。备份从 SQLite 快照到工件归档全程持有共享操作锁，GC 删除前获取同一把排他锁，因此不会在两者之间移除快照仍引用的文件。恢复后的 SQLite 工件目录与实际文件保持一致。
